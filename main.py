    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                ...
                if domain_size > 1 and domain_size < best_count:
                    best = (i, j)
                    best_count = domain_size
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            assigned_vars, failure = self.remove_domain_row(grid, i, j)
            ...
            assigned_vars, failure = self.remove_domain_column(grid, i, j)
            ...
            assigned_vars, failure = self.remove_domain_unit(grid, i, j)
...
class Backtracking:
    ...
    def search(self, grid, var_selector, preprocessed=False): # modified fucn. header to include preprocessed or not check
        """
        ...
        for d in domain:
            ...
            if grid.is_value_consistent(d, i, j):
                new_grid = grid.copy()
                new_grid.get_cells()[i][j] = d

                ac3 = AC3()

                Q = {(i, j)}

                if ac3.consistency(new_grid, Q):
                    continue

                result = self.search(new_grid, var_selector, preprocessed=True)
...
for p in problems:
    ...
    solution = backtracking.search(g, var_selector)
    end_time = time.time()
    ...
    backtracking_fa = Backtracking()

    start_time = time.time()
    solution_fa = backtracking_fa.search(g_fa, var_selector_fa)
    end_time = time.time()
